#!/bin/bash
# Hyprland desktop install — single TUI entrypoint.
#
# Usage:
#   ./arch/hyprland/install.sh
#
# Presents a checkbox menu (gum, falling back to per-item y/n prompts if gum
# isn't installed) of installable components. Only "Hyprland desktop setup"
# and "Disk utility" are pre-selected — everything else is opt-in, so a bare
# run only sets up the desktop itself.
...
source "$REPO_ROOT/arch/components/sddm-numlock.sh"
source "$SCRIPT_DIR/setup.sh"
source "$SCRIPT_DIR/security.sh"

require_not_root
log_section "Hyprland Desktop Install"
...
checkbox_menu "Select components to install" \
    ...
    "hyprland|Hyprland desktop setup|on" \
    "security|Security hardening (ufw + AppArmor)|off"
...
menu_has hyprland       && install_hyprland_component
menu_has security       && install_hyprland_security_component

log_section "Done"
log_success "Selected components installed"
log_info "Log out and back in (or reboot) to start Hyprland via SDDM."
